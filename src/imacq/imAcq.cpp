...
IplImage * imAcqGetImgByCurrentTime(ImAcq * imAcq)
{
    ...
    if (imAcq->lastFrame > 0 && currentFrame > imAcq->lastFrame)
    {
        return NULL;
    }
...
IplImage * imAcqGetImg(ImAcq * imAcq)
{
    ...
    if (imAcq->method == IMACQ_CAM || imAcq->method == IMACQ_VID)
    {
        if (imAcq->width != 0 && imAcq->height != 0)
        {
            // Resize image frome video
            ...
            IplImage* tmp = cvCreateImage(cvSize(imAcq->width, imAcq->height), img->depth, img->nChannels);
            cvResize(img, tmp);
            cvReleaseImage(&img);
            img = tmp;
        }
