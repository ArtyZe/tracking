...
void TLD::fuseHypotheses()
{
    ...
    if (var < min_var)
    {
        //TODO: Think about incorporating this
...
void TLD::initialLearning()
{
    ...
    for (int i = 0; i < detectorCascade->numWindows; i++)
    {
        ...
        if (overlap[i] < 0.2)
        {
            ...
            //TODO: This check is unnecessary if minVar would be set before calling detect.
            if (!detectorCascade->varianceFilter->enabled || variance > detectorCascade->varianceFilter->minVar)
            {
    ...
    for (int i = 0; i < numIterations; i++)
    {
        ...
        detectorCascade->ensembleClassifier->learn(currImg, &detectorCascade->windows[TLD_WINDOW_SIZE * idx],
                                                   true, &detectionResult->featureVectors[detectorCascade->numTrees * idx]);
    }
...
void TLD::learn()
{
    ...
    for (int i = 0; i < detectorCascade->numWindows; i++)
    {
        ...
        if (overlap[i] < 0.2)
        {
            //TODO: Shouldn't this read as 0.5?
            if (!detectorCascade->ensembleClassifier->enabled || detectionResult->posteriors[i] > 0.1)
            {
                negativeIndices.push_back(i);
            }
    ...
    for (size_t i = 0; i < negativeIndices.size(); i++)
    {
        ...
        //TODO: Somewhere here image warping might be possible
        detectorCascade->ensembleClassifier->learn(currImg, &detectorCascade->windows[TLD_WINDOW_SIZE * idx],
                                                   false, &detectionResult->featureVectors[detectorCascade->numTrees * idx]);
    }
