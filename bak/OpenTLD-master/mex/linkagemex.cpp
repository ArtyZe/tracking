...
)
{
    ...
    else if (strcmp(method, "ce") == 0)
    {
        method_key = centroid;
    }
    else if (strcmp(method, "me") == 0)
    {
        method_key = median;
    }
    else if (strcmp(method, "wa") == 0)
    {
        method_key = ward;
    }
    ...
    if (call_pdist)
    {
        ...
        for (j = 0; j < nPdistExtraArgs; j++)
        {
            pdist_rhs[1 + j] = mxGetCell(prhs[2], j);
        }
        ...
        if (!no_squared_input) for (i = 0; i < n; i++)
            {
                y[i] = y[i] * y[i];
            }
    ...
    if (call_pdist)
    {
        mxDestroyArray(pdist_lhs[0]);    /* destroy my pairwise distances or ... */
    }
    else
    {
        mxFree(y);    /* ... or the copy of them */
    }
